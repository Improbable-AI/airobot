...
class Grasper(object):
    ...
    def __init__(self,
        ...
        self.robot = ar.create_robot('ur5e', pb=False,
                                     robot_cfg={'use_cam': True})
        self.robot.gripper.activate()
        ...
        self.T_b_c = np.array(calib_data['b_c_transform']['T'])
    ...
    def reset(self):
        ...
        :rtype: bool
        """
        success = self.robot.set_jpos(self.retract_position)
        self.robot.gripper.open()
        return success

    def _process_depth(self, cur_depth=None):
        if cur_depth is None:
            _, cur_depth = self.robot.camera.get_images(get_rgb=False, get_depth=True)
    ...
    def _get_3D_camera(self, pt, norm_z=None):
        ...
        u = pt[1]
        v = pt[0]
        P = copy.deepcopy(self.robot.camera.cam_P)
    ...
    def _convert_frames(self, pt):
        pt = np.array([pt[0], pt[1], pt[2], 1]).reshape(-1, 1)
        base_pt = np.dot(self.T_b_c, pt).flatten()[:3]
        # assert len(pt) == 3
    ...
    def grasp(self, grasp_pose):
        ...
        euler = [np.deg2rad(180), 0, np.deg2rad(-90) - grasp_angle]
        result = self.robot.set_ee_pose(pregrasp_position, euler)
        ...
        result = self.robot.move_ee_xyz([0, 0, self.grasp_height - self.pregrasp_height])
        ...
        rospy.loginfo('Going to pre-grasp pose')
        result = self.robot.move_ee_xyz([0, 0, self.pregrasp_height - self.grasp_height])
        # result = self.set_pose(pregrasp_position, roll=grasp_angle)
        if not result:
            return False
        time.sleep(self._sleep_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_samples', help='Number of samples for a single grasp inference', type=int,
                        default=N_SAMPLES)
